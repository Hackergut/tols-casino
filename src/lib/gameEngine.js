...
export const PLINKO_MULTIPLIERS = Object.fromEntries(
  Object.entries(PLINKO_SHAPES).map(([k, shape]) => [k, scaleToRtp(shape, PLINKO_PROBS, 0.99)])
);
...
export function kenoPaytable() {
  ...
  const s = ev > 0 ? 0.96 / ev : 0;
  const table = {};
  for (let m = 0; m <= 10; m++) table[m] = Math.max(0, +(KENO_SHAPE[m] * s).toFixed(2));
  return table;
}

// ── Mines (25 tiles, k safe reveals, m mines) — 99% RTP ────────────────────

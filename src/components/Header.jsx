...
export default function Header({ onMenu }) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-md border-b border-white/10" style={{ paddingTop: "env(safe-area-inset-top)" }}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 h-16 flex items-center gap-3">
        {onChild ? (
          <button onClick={() => navigate(-1)} className="lg:hidden grid place-items-center w-9 h-9 rounded-lg bg-card border border-white/10 text-white/70" aria-label="Go back"><ArrowLeft className="w-5 h-5" /></button>
        ) : (

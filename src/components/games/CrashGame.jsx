...
export default function CrashGame() {
  ...
  // compute crash point: 99/(1-r) capped
  const computeCrash = async () => {
    const r = await pf.roll();
    // house edge 1%
    const cp = Math.max(1.0, Math.floor((99 / (1 - r)) * 100) / 100);
    return cp === Infinity ? 1000 : cp;
  };
  ...
  const play = async () => {
    ...
    const tick = (now) => {
      ...
      if (m >= cp) {
        ...
        recordBet({ game_id: "crash", game_name: "Crash Tols", amount, multiplier: 0, payout: 0, result: "lose", client_seed: pf.clientSeed, server_seed_hash: pf.serverHash, nonce: pf.nonce });
        return;
      }
      if (!cashed && autoCashout && m >= autoCashout) {
        doCashout(m);
      }
  ...
  const doCashout = (m = multiplier) => {
    if (cashed || !running) return;
    setCashed(true);
    const payout = amount * m;
    updateBalance(payout - amount, amount);
    recordBet({ game_id: "crash", game_name: "Crash Tols", amount, multiplier: m, payout, result: "win", client_seed: pf.clientSeed, server_seed_hash: pf.serverHash, nonce: pf.nonce });
  };
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_320px] gap-6">
      <div className="space-y-4">
        ...
        {running && (
          <button onClick={() => doCashout()} disabled={cashed} className="w-full h-14 rounded-xl bg-white/5 border border-lime/40 text-lime font-black text-lg hover:bg-lime/10 transition disabled:opacity-40">

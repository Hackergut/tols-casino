...
import { Shield, TrendingUp, Wallet, Users, Activity, DollarSign, ArrowDownToLine, BarChart3, Clock, Ban, RefreshCw, Play } from "lucide-react";
...
import RedeemAdmin from "@/components/admin/RedeemAdmin";
...
import { getMockAdminData, DEMO_ADMIN_USER } from "@/lib/mockAdmin";
...
export default function Admin() {
  ...
  const applyDatasets = (betList, walletList, withdrawalList, userList, earningList) => {
      setBets(betList || []);
  ...
  const loadMock = useCallback(() => {
      ...
      applyDatasets(m.bets, m.wallets, m.withdrawals, m.users, m.earnings);
      setDemoMode(true);
      setAuthed(true);
      // still compute stats from mock
      const betList = m.bets, walletList = m.wallets, withdrawalList = m.withdrawals, userList = m.users, earningList = m.earnings;
      ...
      (betList || []).forEach((b) => {
        ...
        if (idx >= 0 && idx < 12) { buckets[idx].wagered += b.amount || 0; buckets[idx].payout += b.payout || 0; buckets[idx].count += 1; }
      });
      const series = buckets.map((b) => ({ ...b, wagered: +b.wagered.toFixed(2), payout: +b.payout.toFixed(2) }));
      const gameMap = {};
      (betList || []).forEach((b) => { const name = b.game_name || b.game_id || "unknown"; if (!gameMap[name]) gameMap[name] = { name, wagered: 0, count: 0, payout: 0 }; gameMap[name].wagered += b.amount || 0; gameMap[name].payout += b.payout || 0; gameMap[name].count += 1; });
      ...
      const gameLedger = {};
      ledger.forEach((e) => { const n = e.game_name || e.game_id || "unknown"; if (!gameLedger[n]) gameLedger[n] = { name: n, wagered: 0, paid: 0, net: 0, count: 0 }; gameLedger[n].wagered += e.wager || 0; gameLedger[n].paid += e.payout || 0; gameLedger[n].net += e.house_profit || 0; gameLedger[n].count += 1; });
      const ledgerByGame = Object.values(gameLedger).sort((a, b) => b.net - a.net).map((g) => ({ ...g, wagered: +g.wagered.toFixed(2), paid: +g.paid.toFixed(2), net: +g.net.toFixed(2) }));
      setStats({ totalWagered: +totalWagered.toFixed(2), totalPayout: +totalPayout.toFixed(2), houseGross: +houseGross.toFixed(2), houseEdge: totalWagered ? ((houseGross / totalWagered) * 100).toFixed(2) : "0.00", totalBalance: +totalBalance.toFixed(2), totalPlayerWagered: +totalPlayerWagered.toFixed(2), wins, losses, winRate, betCount: (betList || []).length, activeUsers: userList.length, walletCount: (walletList || []).length, pendingWithdrawals: pendingWithdrawals.length, pendingAmount: +pendingAmount.toFixed(2), series, topGames, ledgerWagered, ledgerPaid, ledgerNet, ledgerByGame, ledgerCount: ledger.length });
      setLoading(false);
  }, []);

  const load = useCallback(async () => {
    try {
      // try real backend with 3s timeout
      const authPromise = base44.auth.me();
      const timeout = new Promise((_,rej)=> setTimeout(()=> rej(new Error('auth timeout')), 3000));
      const user = await Promise.race([authPromise, timeout]).catch(()=> null);
      if (!user || user.role !== "admin") {
        // no admin session — try to see if backend is reachable at all
        // if we have mock already or backend unreachable, fall back to demo
        try {
          // quick probe: try one entity list with timeout
          const probe = base44.entities.Bet.list("-created_date", 1);
          const probeTimeout = new Promise((_,rej)=> setTimeout(()=> rej(new Error('probe timeout')), 2000));
          await Promise.race([probe, probeTimeout]);
          // if probe succeeded but user not admin -> show access denied (keep false)
          setAuthed(false);
          setLoading(false);
          return;
        } catch {
      ...
      applyDatasets(betList, walletList, withdrawalList, userList, earningList);
      ...
      console.warn('Admin backend failed, falling back to demo', e);
      loadMock();
    }
  }, [loadMock]);
  ...
  const handleWithdrawal = async (w, action) => {
    try {
      if (w.id.startsWith("local_") || w.id.startsWith("wd_")) {
        // mock: update localStorage
        const hist = JSON.parse(localStorage.getItem("tols_withdrawals") || "[]");
        const next = hist.map((x)=> x.id===w.id? {...x, status: action}: x);
        localStorage.setItem("tols_withdrawals", JSON.stringify(next));
        load();
        return;
      }
      await base44.entities.Withdrawal.update(w.id, { status: action });
      load();
  ...
  const syncCatalog = async () => {
    ...
    try {
      ...
      // demo fallback
      setSyncMsg({ ok: true, text: "Demo: 8 new \u00b7 14 updated \u00b7 156 total (mock)" });
    } finally {
      setSyncing(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#080808] flex items-center justify-center p-6">
        <div className="text-center max-w-md">
          ...
          <p className="text-xs text-white/30 mt-2">No Base44 session — demo available without backend.</p>
          <button onClick={loadMock} className="mt-6 inline-flex items-center gap-2 px-6 h-11 rounded-full bg-lime text-black text-sm font-black hover:opacity-90">
            <Play className="w-4 h-4" /> Enter Demo Admin (mock data)
          </button>
        </div>
      </div>
    );
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight text-white">Admin <span className="text-lime">Dashboard</span>{demoMode && <span className="ml-2 align-middle text-[10px] px-2 py-0.5 rounded-full bg-yellow-500/20 border border-yellow-500/30 text-yellow-300">DEMO MOCK</span>}</h1>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 rounded-2xl border border-white/[0.06] bg-[#121212] p-5">
          ...
          <div className="lg:col-span-2 rounded-2xl border border-white/[0.06] bg-[#121212] p-5">
            ...
            <div className="space-y-1.5 max-h-56 overflow-y-auto scrollbar-hide">
              {stats.ledgerByGame.map((g) => (
                <div key={g.name} className="flex items-center justify-between rounded-lg bg-[#080808] border border-white/[0.06] px-3 py-2">
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-[#121212] p-5">
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto scrollbar-hide">
              {bets.slice(0, 12).map((b) => (
                <div key={b.id} className="flex items-center justify-between rounded-lg bg-[#080808] border border-white/[0.06] px-3 py-2">
                  <div className="flex items-center gap-2">
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-[#121212] p-5">
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto scrollbar-hide">
              {withdrawals.filter((w) => w.status === "pending").map((w) => (
                <div key={w.id} className="flex items-center justify-between rounded-lg bg-[#080808] border border-yellow-500/20 px-3 py-2.5 gap-2">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white">{Number(w.amount).toFixed(2)} {w.currency}</p>
                    <p className="text-xs text-white/40 font-mono truncate">{w.wallet_address}</p>
                    <span className="text-xs font-bold text-yellow-300 uppercase">{w.chain}</span>
                  </div>
                  <div className="flex flex-col gap-1 shrink-0">
                    <button onClick={()=> handleWithdrawal(w, "approved")} className="px-3 py-1 rounded-full bg-lime text-black text-xs font-black">Approve</button>
                    <button onClick={()=> handleWithdrawal(w, "rejected")} className="px-3 py-1 rounded-full bg-red-500/20 border border-red-500/30 text-red-300 text-xs font-bold">Reject</button>
                  </div>
                </div>
              ))}
        ...
        <div className="mt-4"><AffiliateAdmin /></div>
        <div className="mt-4"><BonusAdmin /></div>
        <div className="mt-4"><RedeemAdmin /></div>
      </main>
    </div>
  );
}
...
function Kpi({ icon, label, value, sub, accent }) {
  // Framer motion for KPI cards
  return (
    <motion.div whileHover={{ y: -2 }} transition={{ type: "spring", stiffness: 400, damping: 25 }} className={`rounded-2xl border p-4 sm:p-5 ${accent ? "border-lime/30 bg-gradient-to-br from-lime/10 to-[#111]" : "border-white/10 bg-[#121212]"}`}>
...
function MiniKpi({ label, value, sub, warn }) {
  return (
    <div className={`rounded-xl border p-3 ${warn ? "border-yellow-500/30 bg-yellow-500/5" : "border-white/10 bg-[#121212]"}`}>

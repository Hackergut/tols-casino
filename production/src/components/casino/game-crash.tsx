...
export function CrashGame({ onBack, initialBalance }: Props) {
  ...
  return (
    <div className="space-y-4">
      <style>{`
        .crash-pulse { animation: crashPulse 0.8s ease-in-out infinite; }
        @keyframes crashPulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.85; } }
        .crash-shake { animation: crashShake 0.5s ease-out; }
        @keyframes crashShake {
          0%, 100% { transform: translateX(0); }
          10% { transform: translateX(-12px) rotate(-0.5deg); }
        ...
        .crash-flash-red::after {
          content: ''; position: absolute; inset: 0; background: color-mix(in srgb, var(--loss) 25%, transparent); border-radius: inherit;
          animation: crashFlash 0.6s ease-out forwards; pointer-events: none;
        }
        .crash-flash-lime::after {
          content: ''; position: absolute; inset: 0; background: color-mix(in srgb, var(--color-lime) 16%, transparent); border-radius: inherit;
          animation: crashFlash 0.6s ease-out forwards; pointer-events: none;
        }
        @keyframes crashFlash { 0% { opacity: 1; } 100% { opacity: 0; } }
        .crash-float-win { animation: floatWin 1.2s var(--ease-out-expo) forwards; }
        @keyframes floatWin {
        ...
        .glow-lime { text-shadow: 0 0 20px color-mix(in oklab, var(--color-lime) 60%, transparent), 0 0 40px color-mix(in oklab, var(--color-lime) 30%, transparent), 0 0 80px color-mix(in oklab, var(--color-lime) 15%, transparent); }
        .glow-red { text-shadow: 0 0 20px color-mix(in oklab, var(--color-loss) 60%, transparent), 0 0 40px color-mix(in oklab, var(--color-loss) 30%, transparent); }
        .chart-grid line { stroke: rgba(255,255,255,0.06); stroke-width: 1; }
        .chart-axis text { fill: rgba(255,255,255,0.3); font-size: 10px; }
      `}</style>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
        ...
        <div className="lg:col-span-3">
          <div
            ...
            } ${!reduced && phase === 'crashed' ? 'crash-shake crash-flash-red' : ''} ${!reduced && phase === 'cashed' ? 'crash-flash-lime' : ''}`}
            style={{ minHeight: 320 }}
          >
            ...
            <div className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-center">
              <div className={phase === 'running' && !reduced ? 'crash-pulse' : ''} style={{ transform: `scale(${multScale})`, transition: 'transform 120ms linear' }}>
                <div
              ...
              {phase === 'cashed' && (
                <div className="relative">
                  ...
                  <span className={`absolute -top-2 left-1/2 -translate-x-1/2 font-mono text-lg font-black tabular-nums text-win ${reduced ? '' : 'crash-float-win'}`}>
                    +${payout.toFixed(2)}
                  </span>
                </div>
              )}
              ...
              <svg viewBox={`0 0 ${W} ${H}`} className="h-full w-full" preserveAspectRatio="xMidYMid meet">
                ...
                <g className="chart-axis">
                  {axisLabels.xLabels?.map((label, i) => {
                    const x = PAD + ((i + 1) / (axisLabels.xLabels.length + 1)) * (W - PAD * 2);
                    return <text key={label} x={x} y={H - 10} textAnchor="middle">{label}</text>;
                  })}
                  {axisLabels.yLabels?.map((label, i) => {
                    const y = H - PAD - ((i + 1) / (axisLabels.yLabels.length + 1)) * (H - 80);
                    return <text key={label} x={PAD - 5} y={y + 3} textAnchor="end">{label}</text>;
                  })}
                </g>
                {/* Axis lines */}
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-border/40 bg-surface p-4">
            <p className="mb-2 text-xs text-muted-foreground">Bet Amount</p>
            <div className="mb-3 flex flex-wrap gap-1.5">
              {QUICK_BETS.map(v => (
                <button
                  key={v}
                  onClick={() => setBetAmount(v)}
                  className={`btn-press rounded-lg border px-3 py-1.5 font-mono text-xs font-medium transition-colors duration-[var(--dur-fast)] ${
                    betAmount === v ? 'border-lime/30 bg-lime/15 text-lime' : 'border-transparent bg-secondary/50 text-muted-foreground hover:text-foreground'
                  }`}
                >
                  ${v}
                </button>
            ...
            <div className="flex gap-2">
              <button onClick={() => setBetAmount(a => Math.max(1, Math.floor(a / 2)))} className="btn-press rounded-lg bg-secondary/60 px-3 py-2 text-xs font-bold text-muted-foreground transition-colors hover:text-foreground" disabled={phase === 'running'}>½</button>
              <input
                type="number"
                value={betAmount}
                onChange={e => setBetAmount(Math.max(0, Number(e.target.value)))}
                className="flex-1 rounded-lg border border-border/60 bg-secondary/40 px-3 py-2 text-center font-mono text-sm font-medium tabular-nums text-foreground outline-none focus:border-lime/40"
                disabled={phase === 'running'}
              />
              <button onClick={() => setBetAmount(a => Math.min(balance, a * 2))} className="btn-press rounded-lg bg-secondary/60 px-3 py-2 text-xs font-bold text-muted-foreground transition-colors hover:text-foreground" disabled={phase === 'running'}>2×</button>
            </div>
          </div>

...
const SEGMENT_DEFS = {
  low: [
    { mult: 1.2, color: '#1a3a2a', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 1.5, color: '#2a4a1a', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 1.2, color: '#1a3a2a', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
  ...
  medium: [
    ...
    { mult: 1.5, color: '#2a4a1a', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 3.0, color: '#ff8c00', accent: true },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 1.5, color: '#2a4a1a', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 2.0, color: 'var(--color-lime)', accent: true },
  ...
  high: [
    ...
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 9.9, color: '#ff2d55', accent: true },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    ...
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 4.5, color: '#ff8c00', accent: true },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
    { mult: 0, color: 'var(--color-surface-raised)', accent: false },
...
export function WheelGame({ onBack, initialBalance }: Props) {
  ...
  const confettiParticles = useMemo(() => {
    ...
    return Array.from({ length: 40 }, (_, i) => ({
      ...
      color: ['var(--color-lime)', '#ff8c00', '#ff2d55', '#00d4ff', '#ffffff', '#ffd700'][Math.floor(Math.random() * 6)],
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
        ...
        <div className="rounded-2xl p-6 flex flex-col items-center relative overflow-hidden" style={{ background: 'linear-gradient(135deg, var(--color-surface) 0%, var(--color-bg) 100%)', border: '1px solid color-mix(in oklab, var(--color-lime) 8%, transparent)' }}>
          ...
          <div className="relative z-10" style={{ width: `${SIZE}px`, height: `${SIZE}px`, filter: blurred ? 'blur(1.6px)' : 'none', transition: 'filter 500ms ease' }}>
            ...
            >
              ...
              <text x={CENTER} y={CENTER - 4} textAnchor="middle" dominantBaseline="central" fill="var(--color-lime)" fontSize="9" fontWeight="900" letterSpacing="1.5" fontFamily="system-ui, sans-serif">GOLDEN</text>
              <text x={CENTER} y={CENTER + 8} textAnchor="middle" dominantBaseline="central" fill="var(--color-lime)" fontSize="9" fontWeight="900" letterSpacing="1.5" fontFamily="system-ui, sans-serif">X</text>
            </motion.svg>
          </div>
        ...
        <div className="space-y-3">
          ...
          {/* Bet Amount */}
          <div className="rounded-2xl p-4" style={{ background: 'var(--color-surface)', border: '1px solid color-mix(in oklab, var(--color-lime) 8%, transparent)' }}>
            <p className="text-[10px] uppercase tracking-widest mb-2.5" style={{ color: 'rgba(255,255,255,0.3)' }}>Bet Amount</p>
            <div className="flex gap-1.5 mb-3 flex-wrap">
              {QUICK_BETS.map(v => (
                <button
                  key={v}
                  onClick={() => !spinning && setBetAmount(v)}
                  className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all"
                  style={
                    betAmount === v
                      ? { background: 'color-mix(in oklab, var(--color-lime) 12%, transparent)', color: 'var(--color-lime)', border: '1px solid color-mix(in oklab, var(--color-lime) 30%, transparent)' }
                      : { background: 'rgba(255,255,255,0.03)', color: 'rgba(255,255,255,0.4)', border: '1px solid transparent' }
                  }
                  disabled={spinning}
                >
            ...
            <div className="flex gap-2">
              <button
                onClick={() => !spinning && setBetAmount(v => Math.max(1, Math.floor(v / 2)))}
              ...
              />
              <button
                onClick={() => !spinning && setBetAmount(v => Math.min(balance, v * 2))}
                className="px-3 py-2.5 rounded-lg text-xs font-bold transition-colors"
                style={{ background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.08)' }}
                disabled={spinning}
              >2×</button>
          ...
          <button
            ...
            style={{
              background: spinning
                ...
                : 'linear-gradient(135deg, var(--color-lime) 0%, #a8d600 100%)',
          ...
          <div className="rounded-2xl p-4" style={{ background: 'var(--color-surface)', border: '1px solid color-mix(in oklab, var(--color-lime) 8%, transparent)' }}>
            ...
            <div className="space-y-1.5">
              {risk === 'low' && (
                <>
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-sm" style={{ background: '#1a3a2a' }} /><span className="text-[11px]" style={{ color: 'rgba(255,255,255,0.5)' }}>1.2x — Frequent</span></div>
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-sm" style={{ background: '#2a4a1a' }} /><span className="text-[11px]" style={{ color: 'rgba(255,255,255,0.5)' }}>1.5x — Common</span></div>
              ...
              {risk === 'medium' && (
                <>
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-sm" style={{ background: '#2a4a1a' }} /><span className="text-[11px]" style={{ color: 'rgba(255,255,255,0.5)' }}>1.5x — Frequent</span></div>
                  ...
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-sm" style={{ background: '#ff8c00' }} /><span className="text-[11px] font-bold" style={{ color: '#ff8c00' }}>3.0x — Rare!</span></div>
                </>
              )}
              {risk === 'high' && (
                <>
                  ...
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-sm" style={{ background: '#ff8c00' }} /><span className="text-[11px]" style={{ color: 'rgba(255,255,255,0.5)' }}>4.5x — Rare</span></div>
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-sm" style={{ background: '#ff2d55' }} /><span className="text-[11px] font-bold" style={{ color: '#ff2d55' }}>9.9x — Legendary!</span></div>
                </>
              )}
            </div>
          </div>
      ...
      <style>{`
        ...
        .animate-result-win { animation: result-pop 0.4s ease-out; }
        .animate-result-lose { animation: result-pop 0.3s ease-out; }
      `}</style>
    </div>
  );
}

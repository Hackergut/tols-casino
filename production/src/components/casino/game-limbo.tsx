                  ...
                  {Array.from({ length: 12 }, (_, i) => {
                    ...
                    return (
                      <circle
                        ...
                        fill={['var(--color-lime)', 'var(--win)', '#ffffff', '#ffd700'][i % 4]}
          ...
          <div className="rounded-2xl p-4" style={{ background: 'var(--color-surface)', border: '1px solid color-mix(in oklab, var(--color-lime) 8%, transparent)' }}>
            ...
            <div className="flex gap-1.5 mb-3 flex-wrap">
              {QUICK_BETS.map(v => (
                <button
                  key={v}
                  onClick={() => !rolling && setBetAmount(v)}
            ...
            <div className="flex gap-2">
              <button
                onClick={() => !rolling && setBetAmount(v => Math.max(1, Math.floor(v / 2)))}
                className="px-3 py-2.5 rounded-lg text-xs font-bold transition-colors"
                style={{ background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.08)' }}
                disabled={rolling}
              >½</button>
              ...
              />
              <button
                onClick={() => !rolling && setBetAmount(v => Math.min(balance, v * 2))}
                className="px-3 py-2.5 rounded-lg text-xs font-bold transition-colors"
                style={{ background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.08)' }}
                disabled={rolling}
              >2×</button>

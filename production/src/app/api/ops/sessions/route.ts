...
export async function POST(req: NextRequest) {
  try {
    ...
    // If controlled, we could log it (the controlType is already stored on the session)
    if (controlled) {
      console.log(
        `[CONTROLLED] Session ${session.id} for player ${playerId}: ` +
        `controlType=${controlType || 'unknown'}, outcome=${outcome}, ` +

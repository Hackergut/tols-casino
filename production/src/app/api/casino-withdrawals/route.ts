...
export async function POST(req: NextRequest) {
  ...
  // Hold the funds (decrement now) and record the request as pending.
  const [withdrawal] = await db.$transaction([
    db.casinoWithdrawal.create({
      data: {
        userId: user.id,
        amount,
        currency: "USDT",
    ...
    db.casinoWallet.update({ where: { userId: user.id }, data: { balance: { decrement: amount } } }),
  ]);
